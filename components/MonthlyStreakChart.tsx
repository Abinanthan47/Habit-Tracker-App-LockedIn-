...
import { getDaysInMonth, getMonthName } from '@/lib/dates';
...
export function MonthlyStreakChart({ onDayPress }: MonthlyStreakChartProps) {
  const { completions, cheatDayConfig } = useApp();
  ...
  const calendarData = useMemo(() => {
    ...
    for (let i = 1; i <= days; i++) {
      ...
      // Determine level based on completion count
      let level = 0;
      if (isCheat) {
      ...
      grid.push({ day: i, date: dateStr, level, count, isToday, isFuture });
    }
    
    return grid;
  }, [year, month, completions, cheatDayConfig, today]);
  ...
  const stats = useMemo(() => {
    const activeDays = calendarData.filter(d => d && d.count > 0 && !d.isFuture).length;
    const totalTasks = calendarData.reduce((sum, d) => sum + (d?.count || 0), 0);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.navRow}>
          ...
          <Pressable 
            ...
            style={[styles.navButton, viewDate.getMonth() === new Date().getMonth() && styles.navDisabled]}
          >
      ...
      <View style={styles.statsRow}>
        ...
        <View style={styles.statDivider} />
        <View style={styles.statItem}>
          <Text style={[styles.statValue, { color: '#10B981' }]}>{stats.perfectDays}</Text>
      ...
      <View style={styles.legend}>
        ...
        <View style={[styles.legendDot, { backgroundColor: COLORS.level[0] }]} />
        <View style={[styles.legendDot, { backgroundColor: COLORS.level[1] }]} />
        <View style={[styles.legendDot, { backgroundColor: COLORS.level[3] }]} />
        <View style={[styles.legendDot, { backgroundColor: COLORS.level[5] }]} />
